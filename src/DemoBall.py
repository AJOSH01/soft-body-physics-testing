...
class DemoBlob(App):
    ...
    def Initialize(self):
        ...
        kinex.append(self.world.AddConstraint(outer[-1], outer[0], k))

        # Connect inner skin
        for i in range(1, steps):
            kinex.append(self.world.AddConstraint(inner[i - 1], inner[i], k))
        kinex.append(self.world.AddConstraint(inner[-1], inner[0], k))
        ...
        kinex.append(self.world.AddConstraint(outer[-1], inner[0], k))
    ...
    def Update(self):
        if game.mouse.get_pressed()[0]:
            ...
            if self.grabbed is not None:
                mouse = Vector(*game.mouse.get_pos())
